...
from ..image.preprocess import cast_img_float32
from ..image.preprocess import cast_img_float64
...
def mean_filter(image, kernel_shape, kernel_size):
    ...
    """
    # check parameters
    check_array(image, ndim=2, dtype=[np.float32, np.float64, np.uint8, np.uint16])
...
def median_filter(image, kernel_shape, kernel_size):
    ...
    # get kernel
    kernel = _define_kernel(shape=kernel_shape, size=kernel_size, dtype=image.dtype)
...
def remove_background_mean(image, kernel_shape="disk", kernel_size=200):
    ...
    background = mean_filter(image, kernel_shape=kernel_shape, kernel_size=kernel_size)
...
def dilation_filter(image, kernel_shape=None, kernel_size=None):
    ...
    # check parameters
    check_array(
        image, ndim=2, dtype=[np.uint8, np.uint16, np.float32, np.float64, bool]
    )
    check_parameter(
        kernel_shape=(str, type(None)), kernel_size=(int, tuple, list, type(None))
    )
