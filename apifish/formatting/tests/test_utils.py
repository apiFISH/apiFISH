...
"""
Unitary tests for apifish.image.utils module.
"""
...
def test_check_df():
    ...
    assert stack.check_df(df, features=["B", "A"], features_without_nan=["C", "D", "A"])
    assert stack.check_df(df, features=None, features_without_nan=["A", "C", "D"])
    assert stack.check_df(df, features=["A", "B", "C", "D"], features_without_nan=None)

    # ... and when it should raise an error
    with pytest.raises(ValueError):
        stack.check_df(
            df, features=["A", "B", "C", "D", "E"], features_without_nan=["A", "C", "D"]
        )
    with pytest.raises(ValueError):
        stack.check_df(
            df, features=["A", "B", "C", "D"], features_without_nan=["A", "B", "C", "D"]
        )
