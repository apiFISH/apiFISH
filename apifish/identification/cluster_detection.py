...
def detect_clusters(spots, voxel_size, radius=350, nb_min_spots=4):
    ...
    if ndim not in [2, 3]:
        raise ValueError(
            "Spot coordinates should be in 2 or 3 dimensions, " "not {0}.".format(ndim)
        )
...
def _cluster_spots(spots, voxel_size, radius, nb_min_spots):
    ...
    spots_nanometer = convert_spot_coordinates(spots=spots, voxel_size=voxel_size)
...
def _extract_information(clustered_spots):
    ...
    if clustered_spots.shape[1] == 4:
        ...
        labels_clusters = np.unique(clustered_spots[clustered_spots[:, 3] != -1, 3])
        ...
        for label in labels_clusters:
            spots_in_cluster = clustered_spots[clustered_spots[:, 3] == label, :3]
            z_cluster, y_cluster, x_cluster = spots_in_cluster.mean(axis=0)
            nb_spots_cluster = len(spots_in_cluster)
            clusters.append([z_cluster, y_cluster, x_cluster, nb_spots_cluster, label])
    ...
    # ... and 2-d cluster
        ...
        labels_clusters = np.unique(clustered_spots[clustered_spots[:, 2] != -1, 2])
        if labels_clusters.size == 0:
        ...
        clusters = []
        for label in labels_clusters:
            spots_in_cluster = clustered_spots[clustered_spots[:, 2] == label, :2]
