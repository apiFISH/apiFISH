...
def decompose_dense(
    image, spots, voxel_size, spot_radius, kernel_size=None, alpha=0.5, beta=1, gamma=5
):
    ...
    if beta < 0:
        raise ValueError("'beta' should be a positive value, not {0}".format(beta))
    if gamma < 0:
        raise ValueError("'gamma' should be a positive value, not {0}".format(gamma))
    ...
    parameters_fitted = modelize_spot(
        reference_spot=reference_spot, voxel_size=voxel_size, spot_radius=spot_radius
    )
    if ndim == 3:
    ...
    spots = np.concatenate((spots_out_regions, spots_in_regions[:, :ndim]), axis=0)
...
def simulate_gaussian_mixture(
    ...
    if background < 0:
        raise ValueError("Background value can't be negative: {0}".format(background))
