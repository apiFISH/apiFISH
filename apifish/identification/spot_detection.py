    ...
    if not isinstance(images, list):
        ...
        for i, image in enumerate(images):
            ...
            if i == 0:
                ...
                if ndim != image.ndim:
                    raise ValueError(
                        "Provided images should have the same " "number of dimensions."
                    )
        is_list = True
    ...
    if isinstance(min_distance, (tuple, list)):
        if len(min_distance) != image.ndim:
            raise ValueError(
                ...
                "'min_distance' {1} elements.".format(image.ndim, len(min_distance))
            )
    else:
...
def spots_thresholding(image, mask_local_max, threshold, remove_duplicate=True):
    ...
    if threshold is None:
        ...
        warnings.warn(
            "No spots were detected (threshold is {0}).".format(threshold), UserWarning
        )
        return spots, mask
