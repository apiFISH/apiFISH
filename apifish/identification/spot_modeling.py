...
def modelize_spot(reference_spot, voxel_size, spot_radius, return_coord=False):
    ...
    f = _objective_function(
        ndim=ndim, voxel_size=voxel_size, sigma_z=None, sigma_yx=None, amplitude=None
    )
    if ndim == 3:
...
def initialize_grid(image_spot, voxel_size, return_centroid=False):
    ...
    if image_spot.ndim == 2:
        if return_centroid:
            ...
            grid = _initialize_grid_2d(image_spot, voxel_size[-1], return_centroid)
            return grid
...
def _initialize_grid_3d(image_spot, voxel_size_z, voxel_size_yx, return_centroid=False):
    """Build a grid in nanometer to compute gaussian function values over a
    full volume.
...
def _objective_function(ndim, voxel_size, sigma_z, sigma_yx, amplitude):
    ...
    if ndim == 3:
        ...
        f = _objective_function_2d(
            voxel_size_yx=voxel_size[-1], sigma_yx=sigma_yx, amplitude=amplitude
        )

    return f


def _objective_function_3d(voxel_size_z, voxel_size_yx, sigma_z, sigma_yx, amplitude):
    """Design the objective function used to fit the gaussian function.
    ...
    elif amplitude is not None and sigma_z is not None and sigma_yx is not None:
    ...
    # we fit mu, sigma, amplitude and background
        ...
        def f(grid, mu_z, mu_y, mu_x, sigma_z, sigma_yx, amplitude, background):
            values = gaussian_3d(
                grid=grid,
                mu_z=mu_z,
                mu_y=mu_y,
...
# TODO add equations in the docstring
def gaussian_2d(
    grid, mu_y, mu_x, sigma_yx, voxel_size_yx, amplitude, background, precomputed=None
):
...
def precompute_erf(ndim, voxel_size, sigma, max_grid=200):
    ...
    if ndim not in [2, 3]:
        raise ValueError(
            "Number of dimensions requested should be 2 or 3, " "not {0}.".format(ndim)
        )
...
def _fit_subpixel_2d(image, coord, radius_to_crop, voxel_size_yx, spot_radius_yx):
