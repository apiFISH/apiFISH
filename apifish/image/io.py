...
def read_dv(path, sanity_check=False):
    ...
    if sanity_check:
        check_array(
            video, dtype=[np.uint16, np.int16, np.int32, np.float32], allow_nan=False
        )
...
def read_array_from_csv(path, dtype=None, delimiter=";", encoding="utf-8", skiprows=0):
    ...
    """
    # check parameters
    check_parameter(path=str, dtype=(type, type(None)), delimiter=str, encoding=str)

    # read csv file
    array = np.loadtxt(path, delimiter=delimiter, encoding=encoding, skiprows=skiprows)
    ...
    * A multidimensional boolean image should be saved with
    ...
    if (
        ...
        warnings.warn(
            ...
            "'apifish.stack.save_array' function instead.".format(extension),
            UserWarning,
        )
    if extension in ["tif", "tiff"] and len(image.shape) == 2 and image.dtype == bool:
        raise ValueError(
            "Extension {0} is not fitted with boolean images. "
            "Use 'png', 'jpg' or 'jpeg' extension instead.".format(extension)
        )
    if (
...
def save_data_to_csv(data, path, delimiter=";"):
    ...
    """
    # check parameters
    check_parameter(data=(pd.DataFrame, pd.Series, np.ndarray), path=str, delimiter=str)
    ...
    # save pandas object in a csv file
        ...
        data = data.to_frame()
        data.to_csv(path, sep=delimiter, header=True, index=False, encoding="utf-8")
    else:
        data.to_csv(path, sep=delimiter, header=True, index=False, encoding="utf-8")
