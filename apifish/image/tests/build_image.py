...
def _load_stack(recipe, input_folder, input_dimension=None, i_fov=0):
    ...
    if input_dimension == 2:
        ...
        stack_ = _build_stack_from_4d(recipe, input_folder, fov=i_fov, nb_r=nb_r)
...
def _build_stack_from_3d(recipe, input_folder, fov=0, nb_r=1, nb_c=1):
    ...
    for r in range(nb_r):
        ...
        tensors_3d = []
        for c in range(nb_c):
            path = get_path_from_recipe(recipe, input_folder, fov=fov, r=r, c=c)
            tensor_3d = io.read_image(path)
            tensors_3d.append(tensor_3d)
...
def _load_stack_no_recipe(paths, input_dimension=None):
    ...
    if input_dimension is None:
        testfile = io.read_image(paths[0])
        input_dimension = testfile.ndim

    # get stacks
    stacks = []
    for path in paths:
        s = io.read_image(path)
        stacks.append(s)
