...
@pytest.mark.parametrize("mask_dtype", [np.uint8, np.uint16, np.int32, np.int64, bool])
@pytest.mark.parametrize("spot_dtype", [np.float32, np.float64, np.int32, np.int64])
def test_identify_objects_in_region(ndim, mask_dtype, spot_dtype):
    ...
    # test
    spots_in_, spots_out_ = multistack.identify_objects_in_region(mask, spots, ndim)
